...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cacheRef = ref(db, `cache/${encodeURIComponent(cpf)}`);
    ...
    if (cachedData.exists() && (Date.now() - cachedData.val().timestamp < CACHE_EXPIRATION)) {
      ...
      return res.status(200).json({
        ...
        cached: true,
        requests_remaining: userData.contadores?.cpf || 0
      });
    }
    ...
    const url = `${process.env.DDS}${endpoint}&query=${encodeURIComponent(cpf)}&apikey=${process.env.TKS}`;
